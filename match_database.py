...
class CustomDB:
    ...
    def createFromPath(self, db_custom_path: Path):
        ...
        if os.path.exists(db_custom_path / 'db_img_names.npy') & os.path.exists(CUSTOM_TRAIN / 'embeddings_dict.npy'):
            print('Reload from cache: ', db_custom_path)
            self.img_names = np.load(db_custom_path / 'db_img_names.npy', allow_pickle=True).item()
            for dataset in self.datasets:
                img_paths = [os.path.join(db_custom_path, dataset, 'images', x) for x in self.img_names[dataset]]
        ...
        np.save(db_custom_path / 'db_img_names.npy', self.img_names)
        np.save(db_custom_path / 'embeddings_dict.npy', self.db_embeddings)
    ...
    def checkDesc(self, input_embeddings):
        ...
        # get min key in dis_mean and value
        match_dataset = min(dis_mean, key=lambda k: dis_mean[k][0])
        print(dis_mean[match_dataset])
        if dis_mean[match_dataset][0] > 0.3:
            match_dataset = 'UNKNOWN'
